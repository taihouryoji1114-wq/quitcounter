...
def _render_analysis(period=None):
    ...
    with content:
        ...
        @media(max-width:520px){.statement-grid{grid-template-columns:minmax(105px,1fr) minmax(120px,1.15fr);gap:6px}.surface-card{padding-left:13px!important;padding-right:13px!important}.balance-sheet{grid-template-columns:1fr}.balance-side{padding:12px}.balance-side+ .balance-side{border-left:0;border-top:2px solid #DDE4DF}.balance-side .q-row>div,.balance-side .font-bold,.balance-side .font-black{max-width:58%;font-size:clamp(9px,3vw,12px)!important}.diagnostic-metric{padding:10px}.diagnostic-metric .text-xl,.overview-number{font-size:clamp(11px,3.8vw,16px)!important;letter-spacing:-.03em;text-overflow:clip}}
        """)
