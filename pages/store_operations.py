...
def store_operations_page():
    ...
    with content:
        ...
        open_order_requests = [value for value in order_requests if not value.get("completed")]
        with ui.expansion(f"発注依頼リスト　{len(open_order_requests)}件", icon="add_shopping_cart",
                          value=False).classes("store-panel request-panel w-full q-mt-sm"):
            ui.label("気づいたときに、発注してほしいものを自由に入力").classes(
                "text-[10px] text-grey-6 q-mb-sm")
            request_message = ui.textarea(
                "発注してほしいもの", placeholder="例：ラップが残り少ないので発注お願いします"
            ).props("outlined autogrow").classes("w-full")

            def add_order_request():
                try:
                    store_ops.add_order_request(request_message.value)
                except ValueError as error:
                    ui.notify(str(error), type="negative")
                    return
                reload("発注依頼を追加しました")

            ui.button("発注依頼を追加", icon="send", on_click=add_order_request).props(
                "unelevated no-caps").classes("w-full q-mt-sm")
            if open_order_requests:
                ui.separator().classes("q-my-md")
                for request in open_order_requests:
                    with ui.row().classes(
                            "order-request-row w-full items-start no-wrap "
                            + ("completed" if request.get("completed") else "")):
                        ui.checkbox(
                            value=request.get("completed", False),
                            on_change=lambda event, request_id=request["id"]: (
                                store_ops.set_order_request_completed(
                                    request_id, event.value), reload("発注依頼を更新しました")),
                        ).props("dense")
                        with ui.column().classes("gap-0 grow min-w-0"):
                            ui.label(request["message"]).classes("text-xs font-bold")
                            created = str(request.get("created_at", ""))
                            created_label = created[5:16].replace("T", " ").replace("-", "/")
                            ui.label(f"依頼 {created_label}").classes("text-[8px] text-grey-6")
                        if is_owner:
                            ui.button(
                                icon="delete_outline",
                                on_click=lambda _, request_id=request["id"]: (
                                    store_ops.delete_order_request(request_id),
                                    reload("発注依頼を削除しました")),
                            ).props("flat round dense color=grey-5 aria-label='発注依頼を削除'")
            else:
                ui.label("発注依頼はありません").classes(
                    "w-full text-center text-xs text-grey-6 q-py-md")
                    ...
                    for item in category_items:
                        with ui.row().classes("inventory-row w-full items-center no-wrap"):
                            with ui.column().classes("gap-0 inventory-name"):
                                ...
                                checked_at = str(item.get("last_inventory_check_at", ""))
                                checked_label = (checked_at[:10][5:].replace("-", "/")
        ...
        with ui.expansion("今日の仕込みチェック表", icon="soup_kitchen", value=False).classes(
            ...
            for item in prep_items:
                ...
                row_class = "inventory-row prep-row w-full items-center no-wrap"
                if completed:
                    row_class += " prep-completed"
                with ui.row().classes(row_class) as prep_row:
                    with ui.column().classes("gap-0 grow"):
                        ui.label(item["name"]).classes("prep-item-name text-xs font-black")
                    ...
                    def update_prep_status(event, item_id=item["id"], row=prep_row):
                        store_ops.set_prep_status(
                            today, item_id, "done" if event.value else "incomplete")
                        if event.value:
                            row.classes(add="prep-completed")
                        else:
                            row.classes(remove="prep-completed")

                    ui.checkbox("完了", value=completed, on_change=update_prep_status).props(
                        "dense keep-color color=positive").classes("prep-check")
                    if item.get("source") == "prep" and is_owner:
        ...
        with ui.expansion(f"今日の引き継ぎ　{sum(not value['confirmed'] for value in handovers)}件未確認",
            ...
            for area in ("ホール", "デシャップ", "厨房"):
                ...
                with ui.expansion(f"{area}　{len(area_checks) + len(area_notes)}件", icon="folder",
                    ...
                    render_handover_items(area_checks, area_notes)
        ...
        if has_permission("store_manage"):
            with ui.expansion("店舗運営の設定", icon="settings", value=True).props(
                ...
                prep_templates = store_ops.prep_templates()
                if prep_templates:
                    ui.separator().classes("q-my-md")
                    ui.label(f"登録済みの仕込み項目　{len(prep_templates)}件").classes(
                        "text-xs font-black q-mb-xs")
                    for item in prep_templates:
                        with ui.row().classes("settings-item w-full items-center no-wrap"):
                            with ui.column().classes("gap-0 grow"):
                                ui.label(item["name"]).classes("text-xs font-bold")
        ...
        @media (min-width:700px){
          ...
          .inventory-panel{grid-column:1;grid-row:4}
          .hygiene-panel{grid-column:2;grid-row:2 / span 3;position:sticky;top:18px}
          .prep-panel{grid-column:1;grid-row:5}.handover-panel{grid-column:2;grid-row:5}
          .settings-panel{grid-column:1 / -1;grid-row:6}
          .future-panel{grid-column:1 / -1;grid-row:7}
