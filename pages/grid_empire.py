...
setTimeout(()=>{
 ...
 ]; let cells=[],selected=-1,timer=null,aiTimer=null,ended=false,stageIndex=0;
 ...
 function showHome(){clearInterval(timer);clearInterval(aiTimer);ended=true;document.getElementById('ge-modal').classList.remove('show');game.style.display='none';home.style.display='block'}
 function startStage(i){stageIndex=Number(i);home.style.display='none';game.style.display='block';init()}
 function init(){const s=STAGES[stageIndex];clearInterval(timer);clearInterval(aiTimer);ended=false;selected=-1;cells=Array.from({length:N},(_,i)=>{const land=s.map[Math.floor(i/W)][i%W]==='1';const terrain=!land?'water':i%13===0?'city':i%7===0?'mountain':i%4===0?'forest':'plain';return{o:land?'n':'w',p:land?1+Math.floor(Math.random()*5):0,c:false,t:terrain}});
  ...
  timer=setInterval(grow,900);aiTimer=setInterval(ai,s.speed);
 }
