...
setTimeout(()=>{
 ...
 function tap(i){if(ended||cells[i].o==='w')return;const c=cells[i];if(c.o==='b'){selected=i;state.textContent='兵力 '+c.p;hint.textContent='光る隣接地へ侵攻';render();return}if(selected<0||!neighbors(selected).includes(i))return;attack(selected,i,'b');selected=-1}
