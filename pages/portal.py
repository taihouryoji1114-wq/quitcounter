    ...
    with content:
        with ui.card().classes(
            ...
            with ui.row().classes("w-full items-center no-wrap"):
                with ui.element("div").classes(
                    "w-14 h-14 rounded-xl q-mr-md bg-blue-grey-10 text-amber-4 flex items-center justify-center"
                ):
                    ui.label("CEO").classes("text-xl font-bold")
                with ui.column().classes("gap-0"):
                    ui.label("社長試験").classes("text-xl font-bold")
                    ui.label("会社を守り、育てる経営知識クイズ").classes(
                        "text-grey-7 q-mt-xs"
                    )
                ui.space()
                ui.icon("workspace_premium").classes("text-2xl text-amber-8")
        ...
        with ui.card().classes(
            ...
            with ui.row().classes("w-full items-center no-wrap"):
                ...
                with ui.column().classes("gap-0"):
                    ui.label("ダダダッ大富豪！！！").classes("text-xl font-bold")
                    ui.label("タップで稼ぎ、店と街と人生を豪華にする").classes(
                        "text-grey-7 q-mt-xs"
                    )
                ui.space()
                ui.icon("touch_app").classes("text-2xl text-amber-8")

        with ui.card().classes(
            "habit-card w-full q-pa-lg q-mb-md cursor-pointer"
        ).on("click", lambda _: ui.navigate.to("/static/digital-monsters-v1/index.html?v=14")):
            with ui.row().classes("w-full items-center no-wrap"):
        ...
        with ui.card().classes(
            "habit-card w-full q-pa-lg q-mb-md cursor-pointer"
        ).on("click", lambda _: ui.navigate.to("/static/monster-walk-lab/index.html")):
            with ui.row().classes("w-full items-center no-wrap"):
