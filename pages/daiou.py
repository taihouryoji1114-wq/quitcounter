            ...
            with ui.element('div').classes('world-scroll tokyo-scroll'):
                ...
                for cell in game['map']:
                    ...
                    if cell['owner'] or cell.get('claim'):
                        ...
                        if legion:
                            svg.append(f'''<g class="field-army owner-{marker_owner}" style="color:{NATION_COLORS.get(marker_owner,'#8f7440')}" transform="translate({shape["cx"]-19} {shape["cy"]-5})">
                              ...
                              <path class="army-flag-pole" d="M20-4v-18"/><path class="army-flag" d="M21-21h15l-5 5 5 5H21z"/></g>''')
                        else:
...
(() => {
  requestAnimationFrame(() => {
    ...
    const savedWidth = Math.max(520, Math.min(1700, Number(sessionStorage.getItem('daiou-map-width') || 650)));
    ...
    scroller.addEventListener('touchmove', event => {
      if (event.touches.length === 2 && startDistance) {
        ...
        const nextWidth = Math.max(520, Math.min(1700, startWidth*distance(event.touches)/startDistance));
        const box = scroller.getBoundingClientRect();
        const centerX = (event.touches[0].clientX+event.touches[1].clientX)/2-box.left;
        const centerY = (event.touches[0].clientY+event.touches[1].clientY)/2-box.top;
...
.map-parchment{fill:url(#parchment-depth);stroke:#caa65a;stroke-width:4;filter:url(#map-shadow)}.outer-river{fill:none;stroke:#254d5a;stroke-width:25;opacity:.92}.outer-river+.outer-river{stroke:#376d7a}.river-branch{fill:none;stroke:#417e89;stroke-width:7;opacity:.66}.old-road{fill:none;stroke:#e1c47e;stroke-width:4;stroke-dasharray:3 7;opacity:.55}.mountain-ridge{fill:none;stroke:#b49861;stroke-width:7;stroke-linecap:round;stroke-dasharray:1 10;opacity:.45;filter:drop-shadow(0 4px 2px #050807)}.terrain-detail{pointer-events:none;opacity:.5;stroke:none}.detail-plain{fill:url(#plain-texture)}.detail-forest{fill:url(#forest-texture)}.detail-hill{fill:url(#hill-texture)}.detail-river{fill:url(#river-texture)}.terrain-line,.hill-mark,.water-mark{fill:none;stroke:#f4dda6;stroke-width:1.4;opacity:.48}.terrain-speck,.tree-mark{fill:#e8d39b;opacity:.48}.region-nameplate{pointer-events:none;filter:drop-shadow(0 2px 2px rgba(0,0,0,.8))}.region-nameplate rect{fill:rgba(9,18,16,.82);stroke:rgba(246,218,150,.58);stroke-width:.8}.region-label{fill:#FFF3D0;font-size:10px;font-weight:950;letter-spacing:.04em}.structure-marker{filter:url(#miniature-shadow)}.castle-shadow{fill:#020504;opacity:.72}.castle-base{fill:#5b4a30;stroke:#e8c876;stroke-width:.7}.castle-wall{fill:url(#castle-wall);stroke:#4a351e;stroke-width:.7}.castle-roof{fill:url(#castle-roof);stroke:#422812;stroke-width:.8}.castle-door{fill:#382313}.castle-flag,.army-flag-pole{fill:none;stroke:#f9dc80;stroke-width:1.5}.structure-marker text{font-size:6px;fill:#2d1b0e;stroke:none;font-weight:950}.field-army{filter:url(#miniature-shadow)}.field-army .mini-soldier{stroke:#f7daa0;stroke-width:.65}.field-army .lead{filter:brightness(1.22)}.field-army .helmet{fill:#e9c66e;stroke:#4c331a}.field-army .army-shadow{fill:#020504;stroke:none;opacity:.72}.field-army .army-flag{stroke:#ffe5a0;stroke-width:1;fill:currentColor}.march-route{pointer-events:none;fill:none;stroke:#FFE17C;stroke-width:4;stroke-dasharray:9 7;filter:drop-shadow(0 0 4px rgba(255,209,89,.85));animation:marchFlow 1.1s linear infinite}.march-arrow{fill:#FFE17C}@keyframes marchFlow{to{stroke-dashoffset:-32}}
'''
