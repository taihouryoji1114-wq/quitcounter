...
NATION_COLORS={"n0":"#a88032","n1":"#527ca7","n2":"#337f79","n3":"#a84944","n4":"#aaa68f","n5":"#568947","n6":"#765b94","n7":"#9a693b","n8":"#4c555d","n9":"#7770a2"}
...
def daiou_page():
    ...
    game=games.setdefault(user_id,initial_game()); normalize_game(game); state={"selected":None,"inspect":None,"view":"home","event":None,"tactic":"direct","march":50,"last_move":None,"map_action":None}; data.save()
    ...
    def render():
        ...
        with ui.element('main').classes('daiou-app'):
            ...
            with ui.row().classes('w-full items-center no-wrap map-heading-row'):
                with ui.column().classes('gap-0 grow'):
                    ui.label('葛飾戦図').classes('daiou-heading'); ui.label('2本指で戦図だけを拡大・縮小できます').classes('map-guide')
            ...
            with ui.element('div').classes('world-scroll tokyo-scroll'):
                svg=['''<defs>
                  <marker id="march-arrow" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L7,3 z" class="march-arrow"/></marker>
                  <pattern id="plain-texture" width="54" height="54" patternUnits="userSpaceOnUse"><path d="M5 44 Q18 24 38 32 T53 17" class="terrain-line"/><circle cx="15" cy="15" r="2" class="terrain-speck"/></pattern>
                  <pattern id="forest-texture" width="42" height="48" patternUnits="userSpaceOnUse"><path d="M10 35l8-17 8 17h-5v8h-7v-8zM29 23l5-11 6 11h-4v6h-5v-6z" class="tree-mark"/></pattern>
                  <pattern id="hill-texture" width="70" height="50" patternUnits="userSpaceOnUse"><path d="M2 43 Q20 8 38 43 Q52 18 69 43" class="hill-mark"/></pattern>
                  <pattern id="river-texture" width="64" height="34" patternUnits="userSpaceOnUse"><path d="M0 9 Q16 1 32 9 T64 9 M0 24 Q16 16 32 24 T64 24" class="water-mark"/></pattern>
                  <linearGradient id="parchment-depth" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#314b34"/><stop offset=".48" stop-color="#182c22"/><stop offset="1" stop-color="#0c1917"/></linearGradient>
                  <linearGradient id="castle-roof" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#f6d985"/><stop offset="1" stop-color="#7c4a1c"/></linearGradient>
                  <linearGradient id="castle-wall" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#efe0ba"/><stop offset="1" stop-color="#9b8157"/></linearGradient>
                  <radialGradient id="land-glow"><stop offset="0" stop-color="#ffe4a0" stop-opacity=".18"/><stop offset="1" stop-color="#081210" stop-opacity="0"/></radialGradient>
                  <radialGradient id="war-table-light" cx="48%" cy="42%" r="66%"><stop offset="0" stop-color="#e9c878" stop-opacity=".12"/><stop offset=".72" stop-color="#07110e" stop-opacity=".08"/><stop offset="1" stop-color="#010504" stop-opacity=".72"/></radialGradient>
                  <pattern id="field-lines" width="34" height="34" patternUnits="userSpaceOnUse"><path d="M-5 27Q9 17 39 9M-7 35Q12 22 42 16" class="field-line"/></pattern>
                  <filter id="map-shadow"><feDropShadow dx="0" dy="5" stdDeviation="5" flood-color="#020706" flood-opacity=".75"/></filter>
                  <filter id="miniature-shadow"><feDropShadow dx="0" dy="3" stdDeviation="2" flood-color="#000" flood-opacity=".9"/></filter>
                </defs>
                <rect x="18" y="18" width="684" height="884" rx="32" class="map-parchment"/>
                <ellipse cx="350" cy="408" rx="310" ry="380" fill="url(#land-glow)"/>
                <path d="M62 38 Q32 215 73 377 T52 703 Q40 820 104 903" class="outer-river"/>
                <path d="M655 68 Q691 220 638 367 T664 632 Q690 755 625 895" class="outer-river"/>
                <path d="M44 221 Q83 210 112 232 T174 224 M538 172 Q585 151 651 187 M58 566 Q104 542 156 570 M535 721 Q584 694 658 729" class="river-branch"/>
                <path d="M94 120 Q145 70 198 123 T302 110 M410 116 Q467 63 535 118 T640 103 M101 778 Q155 724 216 782 T328 765 M410 801 Q474 747 544 799 T640 782" class="mountain-ridge"/>
                <path d="M169 103 L342 221 L350 344 L360 468 L369 588 L317 708 L324 828 M526 119 L519 235 L517 355 L508 480 L505 604 L467 716 L461 839" class="old-road"/>''']
                for legion in game.get('legions',[]):
                    ...
                    for delay,size in ((0,5),(-.22,3.7),(-.44,3.2)):
                        svg.append(f'<circle r="{size}" class="marching-unit"><animateMotion begin="{delay}s" dur="2.4s" repeatCount="indefinite" path="M {start["cx"]} {start["cy"]} L {finish["cx"]} {finish["cy"]}"/></circle>')
                drawn_fronts=set()
                for border_cell in game['map']:
                    if not border_cell.get('owner'): continue
                    for enemy in adjacent_cells(game,border_cell):
                        if not enemy.get('owner') or enemy['owner']==border_cell['owner']: continue
                        key=tuple(sorted((border_cell['id'],enemy['id'])))
                        if key in drawn_fronts: continue
                        drawn_fronts.add(key)
                        a=region_shape(border_cell['id']); b=region_shape(enemy['id'])
                        mx=(a['cx']+b['cx'])/2; my=(a['cy']+b['cy'])/2
                        svg.append(f'''<g class="border-clash" transform="translate({mx} {my})">
                          <circle r="11"/><path d="M-6-7L7 7M6-7L-7 7"/><path d="M-8-8l5 1-4 4M8-8l-5 1 4 4"/></g>''')
                for cell in game['map']:
                    ...
                    svg.append(f'<path d="{shape["path"]}" class="terrain-detail detail-{cell["terrain"]}"/>')
                    if cell['terrain']=='plain':
                        svg.append(f'<path d="{shape["path"]}" class="field-overlay"/>')
                    if cell.get('structure') in {'town','capital'}:
                        svg.append(f'''<g class="village-cluster" transform="translate({shape["cx"]+24} {shape["cy"]+20})">
                          <path d="M-19 6h12v9h-12zM-4 1h14v14H-4zM12 7h10v8H12z"/><path d="M-21 6l8-7 8 7M-6 1l9-9 9 9M10 7l7-6 7 6" class="village-roofs"/>
                          <path d="M-24 19q22-7 49 0" class="village-road"/></g>''')
                    svg.append(f'<g class="region-nameplate" transform="translate({shape["cx"]} {shape["cy"]-37})"><rect x="-34" y="-10" width="68" height="16" rx="7"/><text text-anchor="middle" y="2" class="region-label">{cell["name"]}</text></g>')
                    if cell.get('structure'):
                        structure={'capital':'城','town':'町','fort':'砦'}[cell['structure']]
                        svg.append(f'''<g class="structure-marker structure-{cell["structure"]}" transform="translate({shape["cx"]-20} {shape["cy"]-31})">
                          <ellipse cx="20" cy="32" rx="19" ry="5" class="castle-shadow"/>
                          <path d="M5 28h30l-4 6H9z" class="castle-base"/><path d="M9 16h22v13H9z" class="castle-wall"/>
                          <path d="M4 17h32L29 9H11zM9 9h22L26 2H14z" class="castle-roof"/>
                          <rect x="17" y="21" width="6" height="8" class="castle-door"/><path d="M20 2V-7h9l-3 3 3 3h-8" class="castle-flag"/>
                          <text x="20" y="14" text-anchor="middle">{structure}</text></g>''')
                    if cell['owner'] or cell.get('claim'):
                        marker_owner=cell['owner'] or cell['claim']['owner']
                        legion=legion_at(game,cell['id'])
                        if cell.get('claim'):
                            camp_progress=int(cell['claim'].get('progress',0))
                            svg.append(f'''<g class="occupation-camp owner-{marker_owner}" transform="translate({shape["cx"]-22} {shape["cy"]+16})">
                              <ellipse cx="22" cy="12" rx="24" ry="5"/><path d="M4 10L13-5l10 15zM19 10L30-8l12 18z"/>
                              <path d="M43 10V-10M44-10h13l-4 4 4 4H44z" class="camp-standard"/><text x="31" y="18">統治 {camp_progress}/2</text></g>''')
                        if legion:
                            mass='large' if cell['troops']>=18 else ('medium' if cell['troops']>=10 else 'small')
                            extra_left='<g class="mini-soldier flank"><circle cx="1" cy="10" r="3.5"/><path d="M-4 21q5-10 10 0z"/></g>' if cell['troops']>=10 else ''
                            extra_right='<g class="mini-soldier flank"><circle cx="38" cy="10" r="3.5"/><path d="M33 21q5-10 10 0z"/></g>' if cell['troops']>=18 else ''
                            svg.append(f'''<g class="field-army army-mass-{mass} status-{legion.get('status','waiting')} owner-{marker_owner}" style="color:{NATION_COLORS.get(marker_owner,'#8f7440')}" transform="translate({shape["cx"]-19} {shape["cy"]-5})">
                              <ellipse cx="19" cy="19" rx="19" ry="5" class="army-shadow"/>
                              {extra_left}{extra_right}
                              <g class="mini-soldier rear"><circle cx="7" cy="5" r="4"/><path d="M2 18q5-11 10 0z"/><path d="M3 3h8L7-2z" class="helmet"/></g>
                              <g class="mini-soldier lead"><circle cx="19" cy="2" r="5"/><path d="M12 19q7-15 14 0z"/><path d="M14 0h10L19-6z" class="helmet"/></g>
                              <g class="mini-soldier rear"><circle cx="31" cy="6" r="4"/><path d="M26 18q5-11 10 0z"/><path d="M27 4h8l-4-5z" class="helmet"/></g>
                              <path class="army-flag-pole" d="M20-4v-18"/><path class="army-flag" d="M21-21h15l-5 5 5 5H21z"/>
                              <circle cx="19" cy="3" r="25" class="garrison-ring"/></g>''')
                        else:
                ...
                svg.append('<rect x="18" y="18" width="684" height="884" rx="32" class="war-table-vignette"/>')
                markup=(f'<div class="tokyo-map-wrap" data-daiou-map><svg viewBox="{map_viewbox()}" role="img" '
                        f'aria-label="葛飾区内の戦略地図" class="tokyo-map">{"".join(svg)}</svg></div>')
...
(() => {
  requestAnimationFrame(() => {
    ...
    const map = scroller?.querySelector('.tokyo-map');
    if (!scroller || !map || scroller.dataset.gestureBound === '1') return;
    scroller.dataset.gestureBound = '1';
    const savedWidth = Math.max(760, Math.min(1900, Number(sessionStorage.getItem('daiou-map-width') || 1120)));
    map.style.width = `${savedWidth}px`;
    ...
    let startDistance = 0, startWidth = savedWidth, anchorX = 0, anchorY = 0;
    let lastX = 0, lastY = 0, dragged = false;
    const distance = touches => Math.hypot(touches[0].clientX-touches[1].clientX, touches[0].clientY-touches[1].clientY);
    scroller.addEventListener('touchstart', event => {
      dragged = false;
      if (event.touches.length === 2) {
        startDistance = distance(event.touches);
        startWidth = map.getBoundingClientRect().width;
        const box = scroller.getBoundingClientRect();
        const centerX = (event.touches[0].clientX+event.touches[1].clientX)/2-box.left;
        const centerY = (event.touches[0].clientY+event.touches[1].clientY)/2-box.top;
        anchorX = (scroller.scrollLeft+centerX)/startWidth;
        anchorY = (scroller.scrollTop+centerY)/startWidth;
    ...
    scroller.addEventListener('touchmove', event => {
      if (event.touches.length === 2 && startDistance) {
        ...
        const nextWidth = Math.max(760, Math.min(1900, startWidth*distance(event.touches)/startDistance));
        const box = scroller.getBoundingClientRect();
        const centerX = (event.touches[0].clientX+event.touches[1].clientX)/2-box.left;
        const centerY = (event.touches[0].clientY+event.touches[1].clientY)/2-box.top;
        map.style.width = `${nextWidth}px`;
        scroller.scrollLeft = anchorX*nextWidth-centerX;
        scroller.scrollTop = anchorY*nextWidth-centerY;
        sessionStorage.setItem('daiou-map-width', nextWidth);
      } else if (event.touches.length === 1) {
        const x=event.touches[0].clientX, y=event.touches[0].clientY;
        if (Math.abs(x-lastX)+Math.abs(y-lastY)>3) dragged=true;
        scroller.scrollLeft -= x-lastX; scroller.scrollTop -= y-lastY;
        lastX=x; lastY=y;
...
.map-parchment{fill:url(#parchment-depth);stroke:#7e6335;stroke-width:6;filter:url(#map-shadow)}.war-table-vignette{pointer-events:none;fill:url(#war-table-light);stroke:#e0bc67;stroke-opacity:.25;stroke-width:2}.outer-river{fill:none;stroke:#183f4b;stroke-width:28;opacity:.98;filter:drop-shadow(0 3px 2px #030807)}.outer-river+.outer-river{stroke:#285d69}.river-branch{fill:none;stroke:#36717c;stroke-width:9;opacity:.82;filter:drop-shadow(0 2px 1px #07100f)}.old-road{fill:none;stroke:#cdb274;stroke-width:5;stroke-dasharray:2 5;opacity:.55;filter:drop-shadow(0 2px 1px #07100d)}.mountain-ridge{fill:none;stroke:#a88d58;stroke-width:8;stroke-linecap:round;stroke-dasharray:1 9;opacity:.48;filter:drop-shadow(0 4px 2px #050807)}.terrain-detail{pointer-events:none;opacity:.58;stroke:none}.detail-plain{fill:url(#plain-texture)}.detail-forest{fill:url(#forest-texture)}.detail-hill{fill:url(#hill-texture)}.detail-river{fill:url(#river-texture)}.field-overlay{pointer-events:none;fill:url(#field-lines);opacity:.24}.field-line{fill:none;stroke:#f0daa0;stroke-width:.8}.terrain-line,.hill-mark,.water-mark{fill:none;stroke:#f4dda6;stroke-width:1.4;opacity:.48}.terrain-speck,.tree-mark{fill:#e8d39b;opacity:.48}.village-cluster{pointer-events:none;fill:#a98a55;stroke:#362816;stroke-width:.7;filter:drop-shadow(0 2px 2px #020403)}.village-roofs{fill:#34261d;stroke:#d0a85c;stroke-width:.8}.village-road{fill:none;stroke:#c9ad72;stroke-width:2;opacity:.72}.region-nameplate{pointer-events:none;filter:drop-shadow(0 2px 2px rgba(0,0,0,.8))}.region-nameplate rect{fill:rgba(5,13,11,.9);stroke:rgba(246,218,150,.64);stroke-width:.8}.region-label{fill:#FFF3D0;font-size:10px;font-weight:950;letter-spacing:.04em}.structure-marker{filter:url(#miniature-shadow)}.castle-shadow{fill:#020504;opacity:.82}.castle-base{fill:#51432e;stroke:#e8c876;stroke-width:.8}.castle-wall{fill:url(#castle-wall);stroke:#4a351e;stroke-width:.8}.castle-roof{fill:url(#castle-roof);stroke:#2a180d;stroke-width:1.1}.castle-door{fill:#382313}.castle-flag,.army-flag-pole{fill:none;stroke:#f9dc80;stroke-width:1.5}.structure-marker text{font-size:6px;fill:#2d1b0e;stroke:none;font-weight:950}.field-army{filter:url(#miniature-shadow)}.field-army .mini-soldier{stroke:#f7daa0;stroke-width:.65}.field-army .lead{filter:brightness(1.22)}.field-army .helmet{fill:#e9c66e;stroke:#4c331a}.field-army .army-shadow{fill:#020504;stroke:none;opacity:.78}.field-army .army-flag{stroke:#ffe5a0;stroke-width:1;fill:currentColor}.march-route{pointer-events:none;fill:none;stroke:#FFE17C;stroke-width:3.5;stroke-dasharray:8 7;filter:drop-shadow(0 0 4px rgba(255,209,89,.85));animation:marchFlow 1.1s linear infinite}.march-arrow{fill:#FFE17C}@keyframes marchFlow{to{stroke-dashoffset:-30}}
.field-army.army-mass-medium{transform-box:fill-box;transform-origin:center;filter:url(#miniature-shadow) drop-shadow(0 0 3px rgba(255,217,135,.28))}.field-army.army-mass-large{transform-box:fill-box;transform-origin:center;filter:url(#miniature-shadow) drop-shadow(0 0 6px rgba(255,198,94,.42))}.field-army .flank{opacity:.84}.garrison-ring{fill:none;stroke:#f4d27a;stroke-width:1;stroke-dasharray:3 3;opacity:0}.field-army.status-garrisoned .garrison-ring{opacity:.68}.field-army.status-marching .garrison-ring{opacity:0}.occupation-camp{pointer-events:none;fill:#cbb071;stroke:#2b2115;stroke-width:.8;filter:drop-shadow(0 3px 2px #030504)}.occupation-camp ellipse{fill:#090d0b;opacity:.68;stroke:none}.occupation-camp .camp-standard{fill:currentColor;stroke:#ffe6a0;stroke-width:.8}.occupation-camp text{fill:#ffe9b1;stroke:#101412;stroke-width:1.3;paint-order:stroke;font-size:6px;font-weight:950}.border-clash{pointer-events:none;filter:drop-shadow(0 0 5px #dc4d31);animation:clashPulse 1.35s ease-in-out infinite}.border-clash circle{fill:#461c17;stroke:#ffc06e;stroke-width:1}.border-clash path{fill:none;stroke:#ffe0a0;stroke-width:2;stroke-linecap:round}@keyframes clashPulse{50%{opacity:.48;filter:drop-shadow(0 0 10px #ff6847)}}
/* 戦場を主役にした俯瞰型レイアウト。情報カードは戦図の後ろへ退かせる。 */
.daiou-app{display:flex;flex-direction:column;width:100%!important;max-width:980px!important;padding:0 0 54px!important;background:#09100d!important}.daiou-app>.daiou-hero{order:1;min-height:64px!important;margin:0!important;padding:11px 68px 9px!important;border-radius:0!important;background:linear-gradient(180deg,#17241f,#0a110f)!important;border:0!important;border-bottom:1px solid rgba(218,187,112,.3)!important}.daiou-app>.daiou-hero:before{display:none}.daiou-title{font-size:19px!important;letter-spacing:.2em!important;line-height:1}.daiou-copy{display:none}.turn-chip,.identity-chip{top:8px!important;font-size:8px!important;padding:5px 8px!important}.turn-chip{left:55px!important}.identity-chip{right:8px!important}.battle-home-button{left:8px!important;top:8px!important}.daiou-app>.realm-grid{order:2;position:relative;z-index:4;margin:0!important;padding:7px 8px!important;gap:4px!important;background:rgba(7,14,12,.96);border-bottom:1px solid rgba(255,255,255,.08)}.realm-stat{display:flex;align-items:center;justify-content:center;gap:5px;padding:4px 5px!important;border-radius:7px!important;background:rgba(255,255,255,.04)!important;font-size:7px!important}.realm-value{font-size:12px!important}.daiou-app>.command-turn-bar{order:3;margin:0!important;border-radius:0!important;border-width:0 0 1px!important;padding:6px 9px!important}.daiou-app>.war-status{order:4;margin:0!important;border-radius:0!important;padding:6px 9px!important}.daiou-app>.coalition-alert,.daiou-app>.envoy-card{order:5;margin:0!important;border-radius:0!important}.daiou-app>.map-heading-row{order:6;position:absolute;z-index:6;top:151px;left:max(10px,calc((100% - 980px)/2 + 10px));width:auto!important;padding:6px 10px!important;border-radius:7px;background:rgba(5,12,10,.82);box-shadow:0 4px 14px rgba(0,0,0,.35);pointer-events:none}.map-heading-row .daiou-heading{margin:0!important;font-size:12px!important}.map-heading-row .map-guide{display:none}.daiou-app>.nation-legend{order:7;margin:0!important;padding:5px 8px!important;background:#0b1512}.nation-legend-chip{background:rgba(255,255,255,.035)!important;padding:3px 6px!important;font-size:7px!important}.daiou-app>.world-scroll{order:8;height:70vh!important;min-height:560px;max-height:760px!important;margin:0!important;padding:0!important;border:0!important;border-radius:0!important;background:#101a13!important;box-shadow:inset 0 0 40px #010403;scrollbar-width:none}.world-scroll::-webkit-scrollbar{display:none}.tokyo-map{padding:0!important;filter:none!important}.map-parchment{stroke-width:0}.war-table-vignette{stroke-width:0}.daiou-app>.map-source{order:9;margin:0!important;padding:5px 9px;background:#0b1512}.daiou-app>.region-intel{order:10;position:sticky;z-index:7;bottom:0;margin:-68px 8px 8px!important;min-height:60px;background:rgba(10,20,17,.94)!important;backdrop-filter:blur(12px)}.daiou-app>.command-panel{order:11;margin:8px!important}.daiou-app>.army-headquarters{order:12;margin:0 8px 8px!important}.daiou-app>.nation-box{order:13;margin:0 8px 8px!important}.region-shape{stroke-width:.55!important;stroke:rgba(240,226,190,.28)!important}.region-shape.owner-n0{stroke:#f5d277!important;stroke-width:2.4!important}.region-shape.selected{stroke:#fff3b7!important;stroke-width:4!important}.region-nameplate rect{fill:rgba(5,12,9,.72)!important;stroke-opacity:.45}.structure-marker{transform-box:fill-box;transform-origin:center}.structure-capital{filter:url(#miniature-shadow) drop-shadow(0 0 5px rgba(255,191,83,.52))}
@media(max-width:520px){.daiou-app>.world-scroll{height:68vh!important;min-height:510px}.daiou-app>.map-heading-row{top:151px;left:9px}.daiou-app>.daiou-hero{padding-left:58px!important}.army-headquarters{border-radius:12px!important}}
'''
