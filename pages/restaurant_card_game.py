...
def restaurant_card_game_page():
    ...
    state = {"screen": "home", "last_draw": None, "selected_staff": "",
             "business_phase": "formation", "reveal_nonce": 0}
    ...
    def go(screen):
        state["screen"] = screen
        if screen == "business":
            state["business_phase"] = "formation"
        render.refresh()
    ...
    def confirm_formation():
        state["selected_staff"] = ""
        state["business_phase"] = "draw"
        render.refresh()

    def business():
        run_business(profile)
        state["business_phase"] = "result"
        state["reveal_nonce"] += 1
        save()
        render.refresh()
    ...
    def render():
        with ui.element("main").classes("rg-app"):
            ...
            if state["screen"] == "home":
                ...
                phase = state.get("business_phase", "formation")
                ui.label("営業卓").classes("rg-page-title")
                ui.label("配置を決め、中央の山札から襲撃カードを引く").classes("rg-page-copy")
                if phase == "draw":
                    with ui.element("div").classes("rg-table-center"):
                        ui.label("配置確定").classes("rg-table-status")
                        ui.label("中央のカードをタップ").classes("rg-draw-call")
                        with ui.card().classes("rg-attack-deck cursor-pointer").on(
                                "click", lambda _: business()):
                            ui.icon("restaurant_menu").classes("rg-deck-emblem")
                            ui.label("襲撃").classes("rg-deck-title")
                            ui.label("TAP TO DRAW").classes("rg-deck-caption")
                        ui.label("どんなお客様が来るかは、めくるまで分かりません").classes(
                            "rg-table-hint")
                    ui.button("配置をやり直す", icon="undo", on_click=lambda: (
                        state.update(business_phase="formation"), render.refresh())).props(
                            "flat no-caps color=amber-7").classes("w-full q-mt-sm")
                    return
                if phase == "result":
                    result = profile.get("last_result")
                    if result and result.get("customer"):
                        with ui.element("div").classes("rg-table-center result-table"):
                            ui.label("襲撃カード公開！").classes("rg-draw-call")
                            ui.image(result["customer"]["image"]).classes("rg-revealed-card")
                            ...
                            with ui.row().classes("w-full items-center no-wrap q-mb-sm"):
                                ...
                                with ui.column().classes("gap-0 grow"):
                                    ...
                                    ui.label(f"売上候補 {money(result['customer']['sales'])}").classes(
                                        "text-xs font-black")
                            ui.label(f"営業結果　{result['completed']}/4部門達成").classes("font-black")
                        ...
                        ui.button("次の年へ", icon="navigate_next", on_click=lambda: (
                            state.update(business_phase="formation"), render.refresh())).props(
                ...
                ui.button("配置を確定して山札へ", icon="check_circle", on_click=confirm_formation).props(
                    "unelevated no-caps").classes("rg-primary w-full q-mt-md")
