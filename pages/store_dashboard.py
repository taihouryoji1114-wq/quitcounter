...
def store_dashboard_page():
    ...
    with content:
                                    ...
                                    ui.button(icon="check", on_click=lambda _, value=item:
                                              complete_item(value)).props(
                                                  "unelevated round dense aria-label='完了'").classes(
                                                      "board-check")
                                if item["kind"] == "request":
                                    ui.label("長押しで削除").classes("board-longpress-hint")
                    with ui.column().classes("board-lane board-lane-done gap-1"):
                        ...
                        for item in done_items:
                            ...
                            with row:
                                ...
                                ui.label("長押しで戻す" if item["kind"] != "request"
                                         else "長押しで削除").classes("board-longpress-hint")
        ...
        .board-longpress-hint{font-size:6px!important;font-weight:800;line-height:1;color:#87948d;white-space:nowrap}
