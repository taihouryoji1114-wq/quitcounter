...
def store_dashboard_page():
    ...
    with content:
        ...
        with ui.card().classes("store-board w-full q-pa-lg text-white"):
            with ui.row().classes("w-full items-start justify-between no-wrap"):
                with ui.column().classes("gap-0"):
                    ui.label("前日から今日へ").classes("text-[10px] opacity-80 tracking-wide")
            ...
            groups = (
                ("チェック表のやり残し", "checklist",
                 {"prep", "attention", "order_missed", "order_attention"}),
            ...
            for group_label, group_icon, kinds in groups:
                ...
                with ui.column().classes("board-group w-full gap-1 q-mt-md"):
                    ...
                    for item in group_items:
                        attention = item["kind"] in {"attention", "order_attention"}
                        row_class = "board-row attention w-full items-center no-wrap" if attention \
                        ...
                        with ui.row().classes(row_class):
                            ...
                            if item["kind"] in {"prep", "attention"}:
                                ...
                                ui.button("完了", icon="check",
                                          on_click=lambda _, name=destination: (
                                              store_ops.set_daily_order_check(
                                                  board["previous_date"], name, True),
                                              ui.navigate.to("/store-ops")
                                          )).props("unelevated dense no-caps").classes(
                                              "board-complete")
        ...
        ui.add_css("""
        body{background:radial-gradient(circle at 90% 4%,#E3EFE8 0,transparent 27%),linear-gradient(180deg,#F7F9F7,#F3F1EB)!important}.today-ribbon{color:#527060;font-size:9px;font-weight:900;letter-spacing:.14em;margin-bottom:9px;padding-left:4px}.store-board{position:relative;overflow:hidden;border:0!important;border-radius:29px!important;background:radial-gradient(circle at 95% 0%,rgba(234,190,102,.48),transparent 34%),linear-gradient(145deg,#102F26,#286047 62%,#557A4A 120%)!important;box-shadow:0 20px 46px rgba(20,66,49,.28)!important}.store-board:after{content:'';position:absolute;width:180px;height:180px;border:1px solid rgba(255,255,255,.09);border-radius:50%;right:-70px;top:-90px;pointer-events:none}.board-group{padding:10px;border-radius:17px;background:rgba(7,31,24,.16);border:1px solid rgba(255,255,255,.08)}.board-count{min-width:22px;height:22px;display:flex;align-items:center;justify-content:center;border-radius:999px;background:rgba(255,255,255,.17);font-size:9px;font-weight:900}.board-row{padding:9px 9px 9px 11px;border-radius:13px;background:rgba(255,255,255,.94);color:#20362D;margin-top:3px;box-shadow:0 4px 12px rgba(8,31,23,.12)}.board-row.attention{background:#FFF9EA;border:1px solid #EBD8A5}.board-area{min-width:48px;color:#6E8077;font-size:8px;font-weight:900}.board-complete{min-height:32px!important;border-radius:10px!important;background:#246A4E!important;color:white!important;font-size:10px!important;padding:0 10px!important;box-shadow:0 4px 10px rgba(36,106,78,.22)}.board-empty{padding:17px;border-radius:15px;background:rgba(255,255,255,.12);text-align:center;font-size:11px;font-weight:800}.store-app-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.store-app-card{position:relative;overflow:hidden;min-height:164px;border-radius:24px!important;border:1px solid rgba(255,255,255,.8)!important;box-shadow:0 13px 30px rgba(39,55,45,.10)!important;transition:transform .18s,box-shadow .18s!important}.store-app-card:after{content:'›';position:absolute;right:14px;bottom:9px;font-size:29px;font-weight:300;color:rgba(31,54,44,.26)}.store-app-card:nth-child(1){background:linear-gradient(145deg,#EDF5FF,#FFFFFF)!important}.store-app-card:nth-child(2){background:linear-gradient(145deg,#EAF8F3,#FFFFFF)!important}.store-app-card:nth-child(3){background:linear-gradient(145deg,#FFF4E5,#FFFFFF)!important}.store-app-card:nth-child(4){background:linear-gradient(145deg,#F3EEFF,#FFFFFF)!important}.store-app-card:hover{transform:translateY(-3px);box-shadow:0 18px 36px rgba(39,55,45,.15)!important}
        """)
