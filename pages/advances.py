...
def advances_page():
    ...
    with content:
        ...
        with ui.expansion("今月の立替額を入力・修正", icon="edit", value=True).classes("w-full q-mt-md"):
        ...
        with ui.expansion("履歴・累計", icon="history").classes("w-full q-mt-sm"):
            for i, name in enumerate(names):
            ...
            with ui.column().classes("w-full").style("max-height:420px;overflow-y:auto"):
                for month, amounts in sorted(advances.state().get("months", {}).items(), reverse=True):
                    ui.label(f"{month} 立替　合計 ¥{sum(amounts):,}").classes("font-bold q-mt-sm")
