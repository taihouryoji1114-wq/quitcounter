...
<div id="mirai-app">
  ...
  <section class="mk-card result-card">
    ...
    <p class="block-guide">売上から費用を引くたびに箱が小さくなり、最後に残る利益が見える図です。</p>
...
.box-map{display:flex;flex-direction:column;align-items:center;background:#F3F6F4;padding:18px 12px;border-radius:18px}.flow-node{min-width:74px;max-width:100%;box-sizing:border-box;border-radius:14px;padding:12px 6px;color:#fff;text-align:center;transition:width .25s ease}.flow-node span{display:block;font-size:11px;font-weight:800;white-space:nowrap}.flow-node b{display:block;margin-top:2px;font-size:17px;white-space:nowrap}.flow-node em{display:block;margin-top:2px;font-size:9px;font-style:normal;opacity:.9;white-space:nowrap}.flow-sales{background:#355F4C}.flow-gross{background:#4F8C70}.flow-remaining{background:#7DA08E}.flow-profit{background:#3F70B5;box-shadow:0 6px 16px rgba(63,112,181,.22)}.flow-loss{background:#C85C57}.flow-deduction{width:min(94%,560px);display:grid;grid-template-columns:24px 1fr auto;gap:7px;align-items:center;box-sizing:border-box;padding:9px 12px;margin:5px 0;background:#fff;border:1px dashed #CBD4CE;border-radius:12px;color:#5E6C64;font-size:10px}.flow-deduction i{display:flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:#FFF0ED;color:#C85C57;font-style:normal;font-size:16px;font-weight:900}.flow-deduction strong{color:#293A31;font-size:12px}.flow-arrow{height:12px;width:2px;background:#AAB8B0;position:relative}.flow-arrow:after{content:'';position:absolute;bottom:-1px;left:-3px;width:7px;height:7px;border-right:2px solid #AAB8B0;border-bottom:2px solid #AAB8B0;transform:rotate(45deg)}@media(max-width:520px){.box-map{padding:14px 8px}.flow-node{padding:10px 4px}.flow-node span{font-size:9px}.flow-node b{font-size:13px}.flow-deduction{width:96%;padding:8px}}
</style>
            ''',
            sanitize=False,
        ).classes("w-full")
...
(() => {
  function init(){
    ...
    function flowNode(label,value,share,cls,note=''){const width=Math.max(18,Math.min(100,Math.abs(share)*100));return `<div class="flow-node ${cls}" style="width:${width}%"><span>${label}</span><b>${yen(value)}</b>${note?`<em>${note}</em>`:''}</div>`}
    ...
    function update(){
      ...
      const afterPersonnel=gross-personnel,afterPersonnelShare=afterPersonnel/base;
      $('profit-map').innerHTML=`${flowNode('売上',sales,1,'flow-sales','100%')}${deduction('仕入れ・原価',cogs,`原価率 ${pct(costShare)}`)}${flowNode(gross<0?'粗利損失':'粗利',gross,grossShare,gross<0?'flow-loss':'flow-gross',`粗利率 ${pct(grossShare)}`)}${deduction('人件費',personnel,`労働分配率 ${pct(laborShare)}`)}${flowNode('人件費を引いた残り',afterPersonnel,afterPersonnelShare,afterPersonnel<0?'flow-loss':'flow-remaining')}${deduction('家賃',rent,`売上比 ${pct(rent/base)}`)}${deduction('水道光熱費',utilities,`売上比 ${pct(utilities/base)}`)}${deduction('広告費',advertising,`売上比 ${pct(advertising/base)}`)}${deduction('その他管理費',otherExpenses,`売上比 ${pct(otherExpenses/base)}`)}${flowNode(operating<0?'営業損失':'最終的に残る営業利益',operating,profitShare,operating<0?'flow-loss':'flow-profit',`営業利益率 ${pct(profitShare)}`)}`;
