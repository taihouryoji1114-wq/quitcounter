...
<div id="mirai-app">
  <section class="mk-card input-card">
    ...
    <div id="plan-fields" class="input-grid">
      ...
      <details class="target-settings"><summary>判断に使う目標比率</summary><div class="target-grid">
        <label>目標原価率（%）<input id="target-cogs-rate" inputmode="decimal" value="30"></label>
        <label>目標人件費率（%）<input id="target-personnel-rate" inputmode="decimal" value="25"></label>
...
(() => {
  function init(){
    ...
    function houseSvg(sales,gross,operating,parts){
      ...
      const costWidth=sales?Math.max(0,Math.min(300,300*num('cogs')/sales)):0,damage=operating<0?`<polyline points="172,105 185,122 174,140 190,159 178,181 194,205 183,230" fill="none" stroke="#8E2925" stroke-width="5"/>`:'';
      return `<svg viewBox="0 0 360 255" role="img"><polygon class="house-part" points="180,${apexY} 25,105 335,105" fill="${colors.sales}"/><rect class="house-part" x="30" y="93" width="${costWidth}" height="12" rx="4" fill="${colors.cogs}"/><rect x="45" y="105" width="270" height="125" rx="3" fill="#fff" stroke="#42564B" stroke-width="4"/>${rooms}${profitRoom}${damage}<rect class="house-part" x="35" y="${foundationY}" width="290" height="${foundationH}" rx="3" fill="${operating<0?colors.loss:colors.profit}"/><line x1="30" y1="238" x2="330" y2="238" stroke="#42564B" stroke-width="4"/></svg><div class="house-caption">屋根の高さ＝粗利率　部屋の面積＝各経費の負担　土台の厚さ＝営業利益率</div>`;
    }
    ...
    function update(){
      ...
      $('profit-map').innerHTML=houseSvg(sales,gross,operating,parts);
      $('house-legend').innerHTML=legendRow('売上',sales,colors.sales,1)+legendRow('仕入れ・原価',cogs,colors.cogs,costShare,num('target-cogs-rate'))+parts.map(p=>legendRow(p.label,p.value,p.color,p.value/base,p.target)).join('')+legendRow(operating<0?'営業損失':'営業利益',operating,operating<0?colors.loss:colors.profit,profitShare,num('target-operating-rate'),true);
      const issues=[{label:'原価率',diff:costShare*100-num('target-cogs-rate')},{label:'人件費率',diff:personnel/base*100-num('target-personnel-rate')},{label:'家賃比率',diff:rent/base*100-num('target-rent-rate')},{label:'水道光熱費率',diff:utilities/base*100-num('target-utilities-rate')},{label:'広告費率',diff:advertising/base*100-num('target-advertising-rate')}].filter(x=>x.diff>0).sort((a,b)=>b.diff-a.diff);
      const profitGap=num('target-operating-rate')-profitShare*100;if(profitGap>0)issues.push({label:'営業利益率',diff:profitGap});
    ...
    try{const saved=JSON.parse(localStorage.getItem('habitory-future-plan')||'null');if(saved){ids.forEach(id=>{if(saved[id]!==undefined)$(id).value=saved[id]});modes.forEach(id=>{if(saved[id]!==undefined)$(id).value=saved[id]});if(saved['cogs-mode']===undefined)$('cogs-mode').value='amount';if(saved['personnel-mode']===undefined)$('personnel-mode').value='amount';if(saved['other-expenses']===undefined&&saved['other-sga']!==undefined){$('rent').value=0;$('utilities').value=0;$('advertising').value=0;$('other-expenses').value=saved['other-sga']}if(saved.sga!==undefined&&saved.personnel===undefined){$('personnel').value=Math.round(saved.sga*.65);$('other-expenses').value=Math.round(saved.sga*.35)}}}catch(e){}
