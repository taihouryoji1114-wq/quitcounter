...
def render_live_board(business_date, period, period_label):
                            ...
                            if item_type == "status":
                                ...
                                with controls:
                                    ui.button("−", on_click=lambda fn=change_quantity: fn(-1)).props(
                                        "flat round dense aria-label='減らす'").classes("live-quantity-button")
                                    quantity_label.move(controls)
                                    ui.button("＋", on_click=lambda fn=change_quantity: fn(1)).props(
                                        "flat round dense aria-label='増やす'").classes("live-quantity-button")
                                ...
                                with controls:
                                    with ui.button(icon="edit_note").props(
                                            "flat round dense aria-label='補足メモ'").classes(
                                                "live-note-button"):
                                        with ui.menu().classes("live-note-menu"):
                                            note_input = ui.input(
                                            ...
                                            ui.button("完了", on_click=save_note).props(
                                                "unelevated dense no-caps v-close-popup").classes("w-full")
    ...
    .live-board-section{padding-top:15px}.live-board-category{width:100%;padding:0 3px 7px;color:#587064;font-size:11px;font-weight:950;letter-spacing:.08em;border-bottom:2px solid #dce7e1}.live-board-item{height:72px!important;min-height:72px!important;max-height:72px!important;padding:8px 2px;border-bottom:1px solid #edf1ef;overflow:hidden}.live-board-item>.q-row{height:55px!important;min-height:55px!important;max-height:55px!important}.live-board-copy{min-width:0;flex:1;max-height:55px;overflow:hidden}.live-board-name{font-size:14px;font-weight:900;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.live-board-note{display:-webkit-box;margin-top:2px;max-width:100%;max-height:27px;overflow:hidden;color:#a06b18;font-size:10px;font-weight:800;line-height:1.3;overflow-wrap:anywhere;-webkit-box-orient:vertical;-webkit-line-clamp:2}.live-board-controls{gap:5px!important;flex:0 0 auto;margin-left:8px}
    ...
    .live-quantity-button{width:40px!important;height:40px!important;border:1px solid #dce4e0!important;background:#f7f9f8!important;color:#204b3a!important;font-size:19px!important;font-weight:850!important}.live-quantity-value{min-width:66px;text-align:center;font-size:13px;font-weight:950}.live-note-button{width:38px!important;height:38px!important;color:#657a70!important}.live-note-menu{width:min(82vw,320px);padding:13px;border-radius:16px!important}.live-board-empty{padding:28px 3px;color:#78847e;font-size:12px}
