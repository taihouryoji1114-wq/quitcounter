    ...
    with content:
        with ui.card().classes("surface-card w-full q-pa-lg q-mb-xl"):
            ...
            def sync_selected_date():
                record = data.get_workout_for_date(selected_date.value)
            ...
            def save_record():
                ...
                try:
                    data.save_workout(selected_date.value, parts)
        ...
        @ui.refreshable
        def calendar_view():
            records = {record["date"]: record["body_parts"] for record in data.get_workout_records()}
        ...
        @ui.refreshable
        def recent_records():
            records = list(reversed(data.get_workout_records()))
            if not records:
