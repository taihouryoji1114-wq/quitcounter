(() => {
  ...
  function moveUnit(u,p) {
    ...
    u.pos=p; u.moved=true; state.selected=null; log(`${TYPES[u.type].name} ${fmt(u.size)}人が進軍。兵糧を${fmt(cost)}消費。`); render();
  }
  ...
  function attack(u,p,quiet=false) {
    ...
    if(targetU) {
      ...
      targetU.size-=damage;
      if(targetU.size<=0) { state.units=state.units.filter(x=>x!==targetU); if(!quiet)log(`${TYPES[targetU.type].name}を撃破！`); if(targetU.type==='general') finish(u.owner,'敵将を討ち取った'); }
      else if(!quiet) log(`${TYPES[u.type].name}が攻撃。敵兵を${fmt(damage)}人減らした。`);
    } else if(targetB) {
      damage=Math.max(100,Math.round(u.size*.7*(.9+Math.random()*.2))); targetB.hp-=damage;
  ...
  function completePlacement(p) {
    ...
    state.money-=x.cost;
    if(x.kind==='unit') state.units.push({id:`p${Date.now()}`,owner:'player',type:x.type,size:x.size,pos:p,moved:true});
