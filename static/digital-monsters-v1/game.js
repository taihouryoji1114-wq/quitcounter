...
function setPartnerArt(){$('#partner img').src=partnerImages[partner];$('#ally-art').src=partnerImages[partner];$('#ally-name').textContent=names[partner]+'　Lv.5'}setPartnerArt();
...
function startBattle(){if(mode!=='field')return;pos.dx=pos.dy=0;nub.style.transform='none';enemy=100;ally=100;setHp('#ehp',enemy);setHp('#ahp',ally);$('#ally-hp-text').textContent='28 / 28';busy=false;setPartnerArt();show('battle');$('#message').textContent='野生のノイズラットが現れた！'}
...
document.querySelector('[data-hit]').onclick=()=>{if(busy||mode!=='battle')return;busy=true;$('.ally').classList.add('ally-attack');$('#message').textContent=`${names[partner]}のデータパルス！`;setTimeout(()=>{$('.ally').classList.remove('ally-attack');$('.enemy').classList.add('enemy-hurt');$('#battle').classList.add('battle-hit');enemy=Math.max(0,enemy-22);setHp('#ehp',enemy)},330);setTimeout(()=>{$('.enemy').classList.remove('enemy-hurt');$('#battle').classList.remove('battle-hit');if(enemy===0){$('#message').textContent='ノイズラットを倒した！ 研究区へ戻ります。';setTimeout(returnToField,900);return}$('#message').textContent='ノイズラットのグリッチバイト！';$('.ally').classList.add('ally-hurt');ally=Math.max(0,ally-18);setHp('#ahp',ally);$('#ally-hp-text').textContent=Math.ceil(28*ally/100)+' / 28';setTimeout(()=>{$('.ally').classList.remove('ally-hurt');busy=false;$('#message').textContent=ally?'次の行動を選ぼう。':'相棒のデータが停止した……'},430)},760)};
$('#capture').onclick=()=>{if(busy||mode!=='battle')return;busy=true;const chance=enemy<30?.8:enemy<60?.45:.15,success=Math.random()<chance;$('#battle').classList.add('battle-hit');$('#message').textContent=success?'LINK COMPLETE——ノイズラットと接続した！':'LINK ERROR——接続を弾かれた！';setTimeout(()=>{$('#battle').classList.remove('battle-hit');busy=false;if(success)returnToField()},1100)};
