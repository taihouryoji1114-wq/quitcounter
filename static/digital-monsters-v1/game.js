...
function gainExperience(amount){const oldLevel=5+Math.floor(experience/50);experience+=amount;localStorage.setItem('codebeasts:exp',experience);const newLevel=5+Math.floor(experience/50);return newLevel>oldLevel?` ${names[partner]}はLv.${newLevel}になった！`:` 経験値を${amount}獲得。`}
