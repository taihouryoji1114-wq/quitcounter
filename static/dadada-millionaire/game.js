...
const defaults={cash:100,taps:0,earned:100,spent:0,business:{},property:{},owned:[],employees:[],last:Date.now(),sound:true,marketSeed:0,marketMood:1,lifeCat:'すべて',claimed:[]};
...
function income(){let base=businesses.reduce((a,x)=>a+x.income*level('business',x.id),0)+properties.reduce((a,x)=>a+x.income*level('property',x.id),0);return Math.floor(base*(1+s.employees.length*.12))}
...
function tap(e){if(e.target.closest('button'))return;let now=Date.now();combo=now-lastTap<430?Math.min(5,combo+.1):1;lastTap=now;clearTimeout(comboTimer);comboTimer=setTimeout(()=>{combo=1;$('#combo').classList.remove('show');update()},900);$('#combo').classList.add('show');let v=tapValue();s.cash+=v;s.earned+=v;s.taps++;let f=document.createElement('span');f.className='floater';f.textContent='+'+fmt(v);let r=$('.city-stage').getBoundingClientRect();f.style.left=((e.clientX||r.left+r.width/2)-r.left-25)+'px';f.style.top=((e.clientY||r.top+r.height*.65)-r.top)+'px';$('#floaters').appendChild(f);setTimeout(()=>f.remove(),850);beep(480+combo*35);update();if(s.taps%10===0){renderLists();renderRecord()}save()}
function begin(){let old=s.last?Date.now()-s.last:0,gain=Math.min(income()*Math.floor(old/1000),income()*14400);$('#splash').style.display='none';$('#app').hidden=false;render();if(gain>=10&&old>60000){s.cash+=gain;s.earned+=gain;$('#offlineCash').textContent='+'+fmt(gain);$('#offlineTime').textContent=`${Math.floor(old/60000)}分ぶんの自動収益`;$('#offline').hidden=false;$('#offline').classList.remove('dismissed')}}
function renderEmployees(){document.querySelectorAll('[data-hire]').forEach((b,i)=>{let role=b.dataset.hire,hired=s.employees.includes(role);b.classList.toggle('hired',hired);b.innerHTML=hired?`<span>👤</span><small>${role}が勤務中</small>`:`<span>＋</span><small>${role}席</small>`;b.onclick=()=>{if(hired){toast(role+'が働いています');return}let cost=1000*Math.pow(5,s.employees.length);if(s.cash<cost){toast('採用には '+fmt(cost)+' 必要です');return}s.cash-=cost;s.spent+=cost;s.employees.push(role);beep(760);toast(role+'担当を採用！ 自動収益 +12%');renderEmployees();render()}})}
$('#start').onclick=begin;$('#officeStage').onclick=tap;$('#computer').onclick=e=>{e.stopPropagation();$('#pcScreen').hidden=false};$('#pcClose').onclick=()=>{$('#pcScreen').hidden=true};$('#back').onclick=()=>location.href='/';$('#sound').onclick=()=>{s.sound=!s.sound;$('#sound').textContent=s.sound?'♪':'×';save()};$('#offlineOk').onclick=()=>{$('#offline').hidden=true;$('#offline').classList.add('dismissed');render()};$('#reset').onclick=()=>{if(confirm('すべての事業と資産を手放して、最初から始めますか？')){localStorage.removeItem(KEY);location.reload()}};
...
setInterval(()=>{let x=income()/10;if(x){s.cash+=x;s.earned+=x;update()}},100);setInterval(save,5000);addEventListener('pagehide',save);$('#sound').textContent=s.sound?'♪':'×';renderEmployees();
})();
